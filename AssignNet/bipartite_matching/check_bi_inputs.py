def check_bipartite_inputs(graph, sink=None, source=None, directed=None, weighted=None, allow_multi=None):
    if type(graph) == dict:
...
def check_edges(graph, sink, source, directed, weighted, allow_multi):
    ...
    if sink is not None and source is not None and directed is not None and (weighted is not None or weighted is None) and allow_multi is not None:
        return True, 'Valid.'
    elif sink is None and source is None and directed is not None and (weighted is not None or weighted is None) and allow_multi is not None:
        return True, 'Valid.'
    else:
        return False, 'Invalid inputs.'
